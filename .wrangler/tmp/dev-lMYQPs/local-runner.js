...
// functions/api/admin/sync-vectors.js
    ...
    const processArticle = /* @__PURE__ */ __name(async (article) => {
      ...
      const visualPromise = !article.visual_vector && article.image_url ? (async () => {
        try {
          ...
          const visualPromise2 = !article.visual_vector && article.image_url ? (async () => {
            try {
              let imgUrl2 = article.image_url;
              if (imgUrl2.startsWith("/")) {
                imgUrl2 = new URL(imgUrl2, "https://opentuware.pages.dev").toString();
              }
              const imgRes2 = await fetch(imgUrl2);
              if (!imgRes2.ok) return null;
              const arrayBuffer2 = await imgRes2.arrayBuffer();
              return NeuralEngine.getVisualEmbedding(env2, arrayBuffer2);
            } catch (e) {
...
// local-runner.js
var local_runner_default = {
  async fetch(request, env2, ctx) {
    const context2 = { request, env: env2, waitUntil: ctx.waitUntil.bind(ctx) };
    return await onRequestGet(context2);
  }
};
...
// .wrangler/tmp/bundle-8DIZIW/middleware-insertion-facade.js
var __INTERNAL_WRANGLER_MIDDLEWARE__ = [
  middleware_ensure_req_body_drained_default
];
var middleware_insertion_facade_default = local_runner_default;
...
// .wrangler/tmp/bundle-8DIZIW/middleware-loader.entry.ts
