          ...
          <div className="flex items-center space-x-8">
            ...
            <div className="hidden md:flex items-center space-x-6 text-sm font-medium text-tuwa-muted">
              ...
              {sections.length > 0 && sections.map((section) => (
                <Link
                  key={section}
                  href={`/category/${toSlug(section)}`}
                  className="hover:text-white transition-colors text-tuwa-muted"
                >
                  {section}
                </Link>
              ))}
          ...
          <div role="navigation" aria-label="Mobile navigation" className="fixed inset-x-4 top-[83px] z-40 rounded-lg bg-[rgba(10,10,11,0.95)] border border-white/5 p-4 shadow-lg md:hidden">
            ...
            {sections.length > 0 && sections.map((section) => (
              <Link
                key={section}
                href={`/category/${toSlug(section)}`}
                className="block px-3 py-2 rounded hover:bg-white/5 transition-colors text-tuwa-muted hover:text-white"
              >
                {section}
              </Link>
            ))}
