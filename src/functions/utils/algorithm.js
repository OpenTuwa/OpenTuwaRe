...
export async function onRequestGet(context) {
  ...
  try {
    ...
    // Return the array of articles to the frontend
    return new Response(JSON.stringify(finalResults), {
      headers: { "Content-Type": "application/json" }
    });
    
  } catch (err) {
    // If it fails, log the error but return an empty array [] so the frontend doesn't crash with ".reduce is not a function"
